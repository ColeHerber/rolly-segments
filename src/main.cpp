...
#define VERSION "1.0.97"   // updated dynamically from python script
...
void setup()
{
  ...
  motor.PID_velocity.output_ramp = 5; //VERY IMPORTANT SAFTEY FEATURE sets jerk to be this rad/s^3 (5 is good)
  motor.velocity_limit = 500; // rad/s, max rpm is about this
...
void loop()
{
  ...
  if (last_commanded_mode.load() != motor.controller)
  {
    ...
    uint controlmode = MotionControlType::velocity;
