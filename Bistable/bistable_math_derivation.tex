...
\begin{enumerate}
  \item \textbf{Q-ratio condition}: The apex height must substantially exceed
    ...
    \cite{qiu2004}. The MATLAB model uses $Q>2.31$ as a numerical guard, but
    the more conservative design check is $Q\gtrsim2.35$.
...
\subsection{Stacked Cell Mechanics}
\label{ssec:stacked}

Zolfagharian et al.~\cite{zolfagharian2025} use a stack of cells to increase
stroke. In the ideal series model used here:
\begin{align}
  d_{f,N} &= N\,d_{f,1} \quad\text{(total stroke)} \label{eq:stackstroke}\\
  d_{s,N} &= N\,d_{s,1} \quad\text{(saddle position scales proportionally)} \label{eq:stacksaddle}
\end{align}
If the cells are identical and snap sequentially, the peak force threshold is
approximately unchanged: $F_{\mathrm{push},N} \approx F_{\mathrm{push},1}$.
...
$A_N\cdot(Nd_p)(Nd_p - Nd_s)(Nd_p - Nd_f) = N^3\,A_N\cdot d_p(d_p-d_s)(d_p-d_f) = F_{\mathrm{push}}$.

The effective stiffnesses scale as:
\begin{equation}
  k_{\mathrm{eff,1},N} = A_N\cdot N d_s \cdot N d_f = \frac{A_1\,d_s\,d_f}{N}
                       = \frac{k_{\mathrm{eff,1},1}}{N}
  \label{eq:keffN}
\end{equation}
Thus adding cells \emph{reduces stiffness} and \emph{lowers natural frequency}:
\begin{equation}
  f_{n,1,N} = \frac{f_{n,1,1}}{\sqrt{N}}
  \label{eq:fnN}
\end{equation}
...
Configuration~2 avoids the tuned-mass-damper anti-resonance problem (see
Section~\ref{ssec:comparison}) because the motor enforces the plate amplitude
...
%%─────────────────────────────────────────────────────────────────────────────
\subsection{Comparison of Cases and Design Guidance}
\label{ssec:comparison}

\subsubsection{Closed-Form Comparison}

Define the off-resonance baseline $T_0 = 2\zeta_h M_P \omega_{n,1}^2 d_s$. Then:
\begin{align}
  \text{Case 1 (tuned):}\quad
    F_0 &\geq 4\zeta_h\zeta_P\,M_P\,\omega_{n,1}^2\,d_s = 2\zeta_P\,T_0 \label{eq:F0_C1_norm}\\
  \text{Case 2 (stiffness-controlled):}\quad
    F_0 &\geq T_0\,\sqrt{(n^2-1)^2 + (2\zeta_P n)^2} \label{eq:F0_C2_norm}
\end{align}

The ratio of Case~2 to Case~1 thresholds is unchanged from the rotating-unbalance
analysis:
\begin{equation}
  \frac{\text{Case 2 threshold}}{\text{Case 1 threshold}}
  = \frac{\sqrt{(n^2-1)^2 + (2\zeta_P n)^2}}{2\zeta_P}
  \approx \frac{n^2 - 1}{2\zeta_P}
  \qquad (n > 1,\;\zeta_P \ll 1)
  \label{eq:ratio_C2_C1}
\end{equation}
...
by more than $5\times$ compared to the tuned case.
...
\begin{enumerate}
  \item \textbf{Tune the plate.} Set the mounting plate's natural frequency to
    match the bistable mechanism's natural frequency by choosing the stiffness of
    the isolators or mounting brackets:
    \begin{equation}
      k_s = M_P\,(2\pi f_{n,1})^2
      \label{eq:ks_design}
    \end{equation}
    Even an approximate match ($n \leq 1.1$) strongly reduces the required
    force compared with a stiff, uncontrolled
    plate.  Note that $M_P$ here is the mass of the \emph{plate and bistable
    assembly only} --- the motor body should be mounted separately to the chassis
    so that its mass does not add to $M_P$.

  \item \textbf{Minimize plate damping $\zeta_P$.} The tuned threshold is
    $4\zeta_h\zeta_P M_P \omega_{n,1}^2 d_s \propto \zeta_P$. Using low-loss-factor
    rubber isolators is preferred, provided the plate does not become too loose
    or poorly constrained for the robot packaging.

  \item \textbf{Verify the gearbox pass-band.} Any gearbox in the drivetrain
    attenuates vibrations above $f_{\mathrm{cut}}$ (\cref{eq:fcut}).  Confirm
    that $f_{n,1} < f_{\mathrm{cut}}$ so that the actuator force is not filtered
    before it reaches the bistable plate.  High-ratio straight-tooth gearboxes
    are particularly susceptible (see Section~\ref{sssec:highratiospur}); a
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Config_2__Prescribed_Base_Motion.png}
\caption{Time-domain snap-through simulation for Configuration~2 (prescribed base
         motion). The plate displacement $x_P(t)$ and relative hook displacement
         $x_\mathrm{rel}(t)$ are plotted; the hook crosses the saddle $d_s$ and
         latches into State~2 without further motor input.}
\label{fig:config2_snap}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Bistable_Excitation__Animation__Config_2_Prescribed_Motion.png}
\caption{Animation snapshot of the Configuration~2 bistable excitation
         (prescribed base motion). The plate oscillates at $f_{n,1}$ and the
         hook builds resonant amplitude until snap-through occurs.}
\label{fig:anim_config2}
\end{figure}
...
Rigid bearing
  & $T_b(\omega) = 1$
  & Valid: $k_b \gg m_e\omega^2$ at 19.7 Hz \\
Rigid motor shaft
  & $\omega_{\mathrm{crit}} \gg \omega$
  & Valid if shaft critical speed $\gg$ 19.7 Hz \\
...
All results from Sections~\ref{ssec:plateTF}--\ref{ssec:comparison} carry over
...
\caption{Proof-mass actuation analysis: minimum proof mass $M_{pm,\min}$ and
         ...
         parameters ($N$ cells, hook mass $m_h$).}
...
%%─────────────────────────────────────────────────────────────────────────────
\section{RS05 Motor --- Specifications and CAN Control for Bistable Excitation}
\label{sec:rs05}
%%─────────────────────────────────────────────────────────────────────────────
...
Manual (Rev.\ 260428)~\cite{robstride2026}.

%%─────────────────────────────────────────────────────────────────────────────
\subsection{Motor Specifications}
\label{ssec:rs05_specs}

\begin{table}[H]
\centering
\caption{RS05 key parameters relevant to bistable excitation.}
\renewcommand{\arraystretch}{1.35}
\small
\begin{tabular}{p{3.4cm} p{3.4cm} p{6.2cm}}
\toprule
\textbf{Parameter} & \textbf{Value} & \textbf{Notes} \\
\midrule
Rated torque          & 1.6\,N$\cdot$m      & At 100\,RPM, 70$\times$70\,mm heat sink \\
Peak torque           & 5.5\,N$\cdot$m      & Short duration only (see overload table) \\
No-load speed         & 480\,RPM $\pm$10\%  & Output shaft; $\approx 50.3$\,rad/s $= 8$\,Hz \\
Torque constant $K_t$ & 0.94\,N$\cdot$m/A$_\text{rms}$ & Converts Iq current to torque \\
Deceleration ratio    & 7.75:1              & Planetary gearbox \\
Drive mode            & FOC                 & Field-oriented control \\
Max phase current     & 11\,A$_\text{pk}$   & Gives peak torque $\approx 11\times0.94/\sqrt{2}\approx7.3$\,N$\cdot$m (limited to 5.5) \\
Rated phase current   & 2.4\,A$_\text{pk}$  & Continuous \\
CAN bus rate          & 1\,Mbps             & CAN 2.0B, 29-bit extended frame \\
Encoder               & 14-bit absolute     & $\pm4\pi$ range (Type~2 update firmware) \\
Velocity feedback     & $\pm50$\,rad/s      & Mapped [0--65535] in Type~2 frame \\
Torque feedback       & $\pm5.5$\,N$\cdot$m & Mapped [0--65535] in Type~2 frame \\
\bottomrule
\end{tabular}
\label{tab:rs05_specs}
\end{table}

%%─────────────────────────────────────────────────────────────────────────────
\subsection{Speed Limit and Implications for Bistable Excitation}
\label{ssec:rs05_speed}

The 480\,RPM rating is a \textbf{shaft angular velocity limit}
($\omega_{\max} = 50.3$\,rad/s), not a frequency limit.  Its effect depends on
whether the shaft undergoes \emph{continuous rotation} or
\emph{oscillating (reciprocating) motion}.

\subsubsection{Rotating Unbalance --- Continuous Rotation (Infeasible)}

A rotating unbalance requires the shaft to spin continuously at the bistable
natural frequency $f_{n,1}$.  To do so:
\begin{equation}
  n_{\mathrm{required}} = f_{n,1} \times 60
\end{equation}
For $f_{n,1} \approx 20$\,Hz (current design parameters): $n = 1200$\,RPM,
which is $2.5\times$ the 480\,RPM limit.  A rotating unbalance is therefore
infeasible on the RS05 at the bistable resonance frequency.

\subsubsection{Oscillating Excitation --- Velocity, Not Frequency, Is the Constraint}

For any \emph{sinusoidal oscillation} (proof-mass or prescribed base motion)
at frequency $f$ and angular shaft amplitude $\Theta$\,(rad):
\begin{equation}
  v_{\mathrm{peak}} = \Theta \cdot 2\pi f \;\leq\; \omega_{\max} = 50.3\;\text{rad/s}
  \label{eq:vpeak}
\end{equation}
The constraint limits the \emph{amplitude}, not the frequency.  At any frequency
...
\begin{equation}
  \delta_{\max}(f) = \Theta_{\max}(f) \cdot r
                   = \frac{\omega_{\max}\,r}{2\pi f}
  \label{eq:deltamax}
\end{equation}
For snap-through the stroke must reach the saddle: $\delta_{\max} \geq d_s$.
...
\paragraph{Numerical example (current design).}
With $m_h = 0.200$\,kg, $N = 5$ cells, $d_{f,1} = 8.25$\,mm:
$d_s \approx 22.5$\,mm, $f_{n,1} \approx 19.7$\,Hz.
\begin{align}
  \Theta_{\max}(f_{n,1}) &= 50.3 / (2\pi \times 19.7) = 0.406\;\text{rad} = 23.3^{\circ} \\
  \delta_{\max}(f_{n,1}) &= 0.406 \times 0.050 = 20.3\;\text{mm} \quad (r = 50\;\text{mm})
\end{align}
Since $d_s = 22.5\,\text{mm} > 20.3\,\text{mm}$, a 50\,mm lever arm is
marginally insufficient.  The minimum lever arm required is:
\begin{equation}
  r_{\min} = 2\pi \times 19.7 \times 0.0225 / 50.3 = 55.4\;\text{mm}
\end{equation}
A lever arm of 56\,mm or longer makes the velocity constraint non-binding.

\paragraph{Config~2: prescribed base motion.}
...
\begin{equation}
  v_{\mathrm{peak}} = \frac{A_{\min}}{r} \cdot \omega_{n,1}
                   = \frac{2.25\,\text{mm}}{50\,\text{mm}} \times 123.8\,\text{rad/s}
                   = 5.57\;\text{rad/s}
                   \ll \omega_{\max}
\end{equation}
Config~2 is within the velocity limit and requires
$\tau = 2\zeta_h\,k_{\mathrm{eff}}\,d_s\,r \approx 0.34$\,N$\cdot$m for a
50\,mm lever arm, about 21\% of rated torque.

%%─────────────────────────────────────────────────────────────────────────────
\subsection{Control Modes}
\label{ssec:rs05_modes}

The run mode is selected by writing parameter \texttt{run\_mode}
(index \texttt{0x7005}) via Communication Type~18, then enabling the motor
with Type~3.

\begin{table}[H]
\centering
\caption{RS05 control modes (parameter \texttt{run\_mode}, index
         \texttt{0x7005}).}
\renewcommand{\arraystretch}{1.35}
\small
\begin{tabular}{c p{3.0cm} p{9.0cm}}
\toprule
\textbf{Value} & \textbf{Mode} & \textbf{Description} \\
\midrule
0 & Operation (MIT-style) & $\tau_{\mathrm{ref}} = K_d(v_{\mathrm{ref}}-v) + K_p(\theta_{\mathrm{ref}}-\theta) + t_{ff}$; set $K_p=K_d=0$ for pure torque feedforward \\
1 & Position PP            & S-curve trajectory to target angle; no mid-move speed changes \\
2 & Velocity               & Speed loop with current limit \\
3 & Current                & Direct Iq command; no position or speed loop \\
5 & Position CSP           & Cyclic synchronous position; velocity-limited \\
\bottomrule
\end{tabular}
\label{tab:rs05_modes}
\end{table}

For sinusoidal bistable excitation, \textbf{mode 0 (operation) or mode 3
(current)} are the applicable choices.

%%─────────────────────────────────────────────────────────────────────────────
\subsection{Commanding a Sinusoidal Torque via CAN}
\label{ssec:rs05_sine}

\subsubsection{No Onboard Sine Generation}

The motor firmware contains no ``sine wave start'' CAN command and no
writable registers for \texttt{sine\_amplitude} or \texttt{sine\_frequency}.
...
\subsubsection{Option A --- Operation Control Mode (Type~1 Frame)}
...
\begin{tabular}{ll}
\toprule
\textbf{Field} & \textbf{Bistable excitation setting} \\
\midrule
$t_{ff}$ [0--65535] $\to$ $[-5.5, +5.5]$\,N$\cdot$m & $\tau_0 \sin(2\pi f_{n,1} t)$, scaled \\
Angle & 0 \\
Velocity & 0 \\
$K_p$ & 0 \\
$K_d$ & 0 \\
...
\subsubsection{Option B --- Current Mode with \texttt{iq\_ref} (Type~18)}

\begin{enumerate}
  \item Write \texttt{run\_mode} = 3 via Type~18 (index \texttt{0x7005}).
  \item Enable motor: send Type~3.
  \item Each tick: write \texttt{iq\_ref} (index \texttt{0x7006}, float,
    $\pm 11$\,A) via Type~18:
    \begin{equation}
      i_q(t) = \frac{\tau_0}{K_t}\,\sin(2\pi f_{n,1}\,t)
      \label{eq:iqref}
    \end{equation}
    For $\tau_0 = 0.34$\,N$\cdot$m and $K_t = 0.94$\,N$\cdot$m/A:
    $i_{q,0} = 0.36$\,A --- well within the $\pm 11$\,A limit.
\end{enumerate}
...
%%─────────────────────────────────────────────────────────────────────────────
\subsection{ESP32 Feasibility Assessment}
\label{ssec:rs05_esp32}

\begin{table}[H]
\centering
\caption{CAN bus load for two RS05 motors commanded at 500\,Hz per motor.}
\renewcommand{\arraystretch}{1.35}
\begin{tabular}{ll}
\toprule
...
\midrule
CAN bus rate            & 1\,Mbps \\
Frame size (extended)   & $\approx 110$\,bits \\
Frame transmission time & $\approx 110\,\mu$s \\
...
\begin{itemize}[nosep]
  \item Disable WiFi/Bluetooth; pin the CAN interrupt and timer callback to
    Core~1 at high FreeRTOS priority to avoid latency spikes.
  \item Use \texttt{esp\_timer\_create} (not \texttt{vTaskDelay}) for the 1\,ms
    tick; jitter is then $<50\,\mu$s.
  \item Accumulate phase as a running sum ($\varphi \mathrel{+}= 2\pi f \Delta t$
    each tick) rather than computing $\sin(2\pi f t_{\mathrm{wall}})$ to avoid
    jitter corrupting the waveform.
  \item Snap detection: read position (Bytes~0--1 of Type~2 feedback) each
    reporting cycle; a position jump of $\geq d_s$ through the lever arm
    signals snap and the torque command should be zeroed.
  \item Enable CAN timeout protection: set \texttt{CAN\_TIMEOUT} $> 0$ so the
...
\begin{subfigure}[b]{0.49\textwidth}
  ...
  \caption{$M_{pm,\min}$ vs.\ drive frequency and number of cells $N$.}
...
\begin{subfigure}[b]{0.49\textwidth}
  ...
  \caption{Required torque vs.\ drive frequency and number of cells $N$.}
...
\subsection{Stacked-Cell Scaling (\texorpdfstring{$N$}{N} cells)}
\label{ssec:ncells}

Adding bistable cells in series scales the mechanism as follows:
\begin{align}
  d_f &= N\,d_{f,1} \quad\text{(stroke grows linearly)}\\
  k_{\mathrm{eff},1} &\propto \frac{1}{N} \quad\text{(stiffness drops)}\\
  f_{n,1} &\propto \frac{1}{\sqrt{N}} \quad\text{(natural frequency drops)}\\
  F_{0,\min} &\approx \text{const} \quad\text{(independent of $N$, see \cref{eq:F0min2})}
\end{align}
The last result follows because $k_{\mathrm{eff},1} \propto 1/N$ and
$d_s \propto N$, so their product $k_{\mathrm{eff},1}\,d_s$ is constant.

The post-snap potential energy available to accelerate the hook to State~2:
\begin{equation}
  \Delta E_2 = V(d_s) - V(d_f)
\end{equation}
grows with $N$ (deeper well, larger stroke).  This is the \emph{impact energy}
...
\begin{table}[H]
\centering
\caption{Effect of increasing each design variable on key performance metrics.
         $\uparrow$ = increases, $\downarrow$ = decreases, $\leftrightarrow$ = unchanged.}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{lcccccc}
\toprule
\textbf{Variable} & $f_{n,1}$ & $d_f$ & $\Delta E_2$ & $F_{0,\min}$
                  & $M_{pm,\min}$ & $\tau_{C2}$ \\
\midrule
$\uparrow m_h$    & $\downarrow$ & $\leftrightarrow$ & $\leftrightarrow$
                  & $\leftrightarrow$ & see note & $\leftrightarrow$ \\
$\uparrow N$      & $\downarrow$ & $\uparrow$ & $\uparrow$
                  & $\leftrightarrow$ & $\downarrow$ & $\leftrightarrow$ \\
$\uparrow \zeta_h$ & $\leftrightarrow$ & $\leftrightarrow$ & $\leftrightarrow$
                  & $\uparrow$ & $\uparrow$ & $\uparrow$ \\
$\uparrow d_{s,\mathrm{frac}}$ & $\downarrow$ & $\leftrightarrow$ & $\downarrow$
                  & $\uparrow$ & $\uparrow$ & $\uparrow$ \\
\bottomrule
\end{tabular}
...
\noindent\textbf{Note on $m_h$:} $M_{pm,\min}$ depends on the full coupled
dynamics.  Heavier hooks lower $f_{n,1}$ and $f_{\mathrm{rel}}$, shifting
...
To \textbf{maximise impact energy}: increase $N$ (more cells, deeper well).

To \textbf{minimise required actuator mass} ($M_{pm}$):
drive near $f_{\mathrm{rel}}$ (avoid TMD anti-resonance), increase $N$
(lower $f_{n,1}$ and $f_{\mathrm{rel}}$, higher $F/M$ from
\cref{eq:force_per_mass}).

To \textbf{minimise motor torque} (Config~2): drive at $f_{n,1}$; torque is
independent of $m_h$ and $N$ (both cancel in \cref{eq:tau_c2}).

To \textbf{ensure bistability}: maintain $Q = h/t \gtrsim 2.35$ and use a
double-beam layout to suppress mode~2.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Comprehensive_Design_Space_vs_n_cells.png}
\caption{Comprehensive design space: key performance metrics (natural frequency
         $f_{n,1}$, energy barrier $\Delta E_1$, impact energy $\Delta E_2$,
         minimum required force $F_{0,\min}$, and minimum proof mass
         $M_{pm,\min}$) as a function of the number of stacked cells $N$.
         All curves are normalised to their single-cell values.}
\label{fig:design_space}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Impact_Energy__Design_Cost_vs_n_cells.png}
\caption{Impact energy $\Delta E_2$ and actuation cost metrics as a function
         of the number of stacked cells $N$.  Impact energy grows with $N$
         (deeper potential well, longer stroke), while the minimum proof mass
         $M_{pm,\min}$ decreases, making additional cells doubly beneficial.}
\label{fig:impact_energy}
\end{figure}
...
$\tau_{C2}$ & N$\cdot$m & Config~2 motor torque $= 2\zeta_h k_{\mathrm{eff}} d_s r$ & 0.34 \\
