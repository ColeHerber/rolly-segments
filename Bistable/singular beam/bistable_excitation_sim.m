...
% Singular-Beam Mass Actuation Force Optimization for Bistable Hook Deployment
%
...
%       │
%     SINGLE BEAM        ← one bistable curved beam (Qiu cosine profile)
%       └──────┬────────┘
%              │
...
% Singular-beam run: displacement, force, and stiffness are all single-beam
% quantities. Use the parent-folder script for parallel beam-pack sweeps.
d_stroke_1cell = h_apex * 2.2;  % [m]  single-beam stroke
d_saddle_frac  = 0.545;         % [-]  saddle fraction of stroke
n_cells        = 1;             % [-]  singular beam
...
fprintf('  F_push total (N=%d singular) = %.2f N\n', n_cells, F_push_total);
fprintf('  d_stroke_1cell            = %.2f mm\n', d_stroke_1cell*1e3);
fprintf('  d_stroke mechanism        = %.2f mm  (single beam)\n', d_stroke*1e3);
fprintf('══════════════════════════════════════════════════════════════\n');
fprintf('\n══════════════════════════════════════════════════════════════\n');
...
tau_opt   = tau_1d(idx_opt);
fprintf('  Optimal: f_opt = %.1f Hz | M_pm_min = %.1f g | Δ = %.2f mm | τ = %.3f N·m\n', ...
        f_opt, M_pm_opt_1d*1e3, Delta_opt*1e3, tau_opt);
...
for nci = 1:Nnc
    ...
    fprintf('  Beam count N=%d: f_n1=%.1f Hz  f_rel=%.1f Hz  KE=%.2f mJ  stroke=%.1f mm\n', ...
...
save(fullfile(fig_dir,'singular_beam_hook_mass_optimization_results.mat'), ...
     'f_sweep_1d','f_grid_vec','mhook_grid_vec','surf_Mpm','surf_tau', ...
     'surf_frel','surf_fn1','M_pm_min_nc','tau_c2_nc','f_opt', ...
     'M_pm_opt_1d','Delta_opt','tau_opt','f_n1','f_rel','d_stroke', ...
     'd_saddle','k_eff_s1','F_push_1cell','F_push_total','n_cells');
fprintf('Saved numeric results: %s\n', fullfile(fig_dir,'singular_beam_hook_mass_optimization_results.mat'));
...
% Highlight current singular-beam design point
...
if Nnc > 1
%% ── Figure 5: 3-D M_pm_min(f_drive, beam count) ─────────────────────
fig5 = figure('Name','3-D M_{pm,min}(f_{drive}, beam count)','NumberTitle','off', ...
...
% Resonance ridge: f_rel(N)
Z5_frel = zeros(1,Nnc);
for nci=1:Nnc, [~,fi_r]=min(abs(f_sweep_1d-frel_nc(nci))); Z5_frel(nci)=M_pm_min_nc(nci,fi_r); end
plot3(frel_nc, n_cells_vec, Z5_frel+0.5,'w-','LineWidth',3,'DisplayName','f_{rel}(N) ridge');
% Current design point
...
xlabel('Drive frequency  (Hz)','FontSize',12);
ylabel('Beam count  N','FontSize',12);
zlabel('Min proof mass  M_{pm,min}  (g)','FontSize',12);
...
%% ── Figure 6: 3-D Torque(f_drive, beam count) — Config 2 (analytical) ─
fig6 = figure('Name','3-D Torque(f_{drive}, beam count) — Config 2','NumberTitle','off', ...
...
% Resonance valley: f_n1(N)
Z6_fn1 = zeros(1,Nnc);
for nci=1:Nnc, [~,fi_n]=min(abs(f_sweep_1d-fn1_nc(nci))); Z6_fn1(nci)=tau_c2_nc(nci,fi_n); end
plot3(fn1_nc, n_cells_vec, Z6_fn1+0.02,'w-','LineWidth',3,'DisplayName','f_{n1}(N) valley');
...
%% ── Figure 7: Comprehensive design optimisation vs beam count ───────
...
fig7 = figure('Name','Comprehensive Design Space vs beam count','NumberTitle','off', ...
...
%% Panel 3: Key frequencies vs beam count
...
sgtitle(sprintf('Comprehensive Design Space vs Beam Count  |  m_{hook}=%.0fg  |  d_{beam}=%.2fmm  |  \\zeta_h=%.2f', ...
        m_hook*1e3, d_stroke_1cell*1e3, zeta_hook), 'FontSize',12,'FontWeight','bold');
else
    fprintf('Skipping beam-count sweep figures for singular-beam run (Nnc = 1).\n');
end
