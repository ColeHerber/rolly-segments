...
if nargin == 0
    ...
    params.n_cells         = 1;        % [-] singular beam
    params.m_hook          = 0.200;    % [kg]
    params.zeta_hook       = 0.02;     % [-]
    params.t_end           = 25.0;     % [s]  search window
    params.F0_lo           = 0.01;     % [N]
    params.F0_hi           = 60.0;     % [N] single-beam run needs >11.5 N near resonance
...
if isfield(params,'plot_results') && params.plot_results
    ...
    fig_dir = fullfile(save_dir,'figures','optimize_displacement_sim2');
...
% vary h_apex (keep t_beam = 0.6mm, singular beam)
p2 = params;
...
% vary t_beam (keep h_apex = 2.5mm, singular beam)
...
% singular-beam comparison point (kept for plot compatibility)
...
xlabel('Beam count','FontSize',10);
title('Single-Beam Comparison Point','FontWeight','bold');
